...
class IVStats(Stats):
    ...
    def __init__(
        self, pocket_monster: "PocketMonster", random_init: bool = False
    ):
        super().__init__(
            pocket_monster=pocket_monster, max_value=31, prefix="iv_"
        )
        if random_init is True:
            ...
            if self[StatsEnum.SPECIAL_ATTACK] is not None:
                init_error += (
                    f"special_attack={self[StatsEnum.SPECIAL_ATTACK]}, "
                )
            if self[StatsEnum.SPECIAL_DEFENSE] is not None:
                init_error += (
                    f"special_defense={self[StatsEnum.SPECIAL_DEFENSE]}, "
                )
